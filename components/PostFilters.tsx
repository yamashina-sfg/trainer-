...
import { bodyPartFilters, sportFilters } from "@/lib/constants";
...
export function PostFilters({ posts }: { posts: Post[] }) {
  ...
  const filteredPosts = useMemo(() => {
    return posts.filter((post) => {
      ...
      return matchesPart && matchesSport;
    });
  }, [bodyPart, posts, sport]);
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {filteredPosts.length > 0 ? (
          ...
          <div className="sfg-card py-10 text-center text-sm text-slate-600">該当する相談はまだありません。</div>
        )}
      </div>
    </div>
  );
